...
def shift_rows(state):
    state[1], state[2], state[3] = state[1][1:], state[2][2:], state[3][3:]
    state[0][1], state[1][2], state[2][3], state[3][0] = state[1][0], state[2][1], state[3][2], state[0][3]

def inv_shift_rows(state):
    state[1], state[2], state[3] = state[1][3:], state[2][2:], state[3][1:]
    state[0][1], state[1][2], state[2][3], state[3][0] = state[1][0], state[2][1], state[3][2], state[0][3]
...
def expand_key(key):
    ...
    expanded_keys = key_matrix[:]
    i = 1
    while len(expanded_keys) < 44:  # AES-256 has 44 words
        temp = expanded_keys[-1]
        if len(expanded_keys) % 8 == 0:
            temp = [s_box[b] for b in temp[1:]] + [s_box[temp[0]]]
            temp[0] ^= r_con[i]
            i += 1
        expanded_keys.append(xor_bytes(expanded_keys[-8], temp))
...
class AES256:
    ...
    def encrypt_block(self, block):
        ...
        sub_bytes(state, s_box)
        shift_rows(state)
        add_round_key(state, self.round_keys[13])
